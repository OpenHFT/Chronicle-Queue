...
A number of relevant system properties are listed in link:src/main/docs/system-properties.adoc[system-properties.adoc].
High-level architectural and operational choices are recorded in link:src/main/docs/decision-log.adoc[decision-log.adoc].

== Building and Testing

Chronicle Queue is built and tested against JDK 8 and JDK 21. Use an explicit `JAVA_HOME` to match the target JDK when running Maven:
...
env JAVA_HOME=/usr/lib/jvm/java-1.8.0-openjdk-amd64 \
    ...
    mvn clean install | tee build-java8.log

# Java 21
env JAVA_HOME=/usr/lib/jvm/java-1.21.0-openjdk-amd64 \
    PATH=/usr/lib/jvm/java-1.21.0-openjdk-amd64/bin:$PATH \
    mvn clean install | tee build-java21.log
----

If you see a warning like `Failed to compile generated method writer - falling back to proxy method writer`, consult the troubleshooting entry in link:src/main/docs/FAQ.adoc[FAQ].
...
* passes messages between JVMs on different machines via replication in under 10 µs
...
When publishing 40-byte messages, a high percentage of the time we achieve latencies under 1 µs.
...
Chronicle Queue aims to achieve latencies of under 40 µs for 99% to 99.99% of the time.
Using Chronicle Queue without replication, we support applications with latencies below 40 µs end-to-end across multiple services.
...
Chronicle Queue supports low latency IPC (Inter Process Communication) between JVMs on the same machine in the order of magnitude of 1 µs; as well as between machines with a typical latency of 10 µs for modest throughputs of a few hundred thousands.
...
This is because the cost of logging is very low; less than 10 µs.
...
NOTE: See Section link:#_excerpt_indexing_in_chronicle_queue[Excerpt indexing in Chronicle Queue] of this User Guide for more information and examples of using indexes.
...
''''
[#_excerpt_indexing_in_chronicle_queue]
...
It is recommended to not reduce the sample rate below 50 µs as this will produce too much noise
...
In the test described above, the typical latency varied between 14 and 40 µs.
The 99 percentile varied between 17 and 56 µs depending on the throughput being tested.
Notably, the 99.93% latency varied between 21 µs and 41 ms, a factor of 2000.

.Possible throughput results depending on acceptable latencies
|===
| Acceptable Latency | Throughput
| < 30 µs 99.3% of the time | 7 million message per second
| < 20 µs 99.9% of the time | 20 million messages per second
| < 1 milliseconds 99.9% of the time | 50 million messages per second
| < 60 µs 99.3% of the time | 80 million message per second
|===
...
While Kafka is faster than many of the alternatives, it doesn't match Chronicle Queue's ability to support throughputs of over a million events per second, while simultaneously achieving latencies of 1 to 20 µs.
