...
Different roll cycles have a difference balance between how many bits we give to the message sequence number and how many of the remaining bits gets allocated to the cycle number, So another words different roll cycle trade off the max number of cycles for the next number of messages within the cycle.
With 32-bit  (4 bn) per day, you can have 31-bit cycles up to the year 5,881,421 though with hourly it runs out in the year 246,947. If you have more than 4 billion messages per cycle you can increase the number of bits used for cycles and thus the number of messages per cycle though reduce the number of cycles.
