...
=== Reading a Queue File for the Command Line

When Chronicle Queue stores is data it creates a file  with an extension of of 'cq4',  
the data that is written to this file, is usually stored in binary format,  like this :
...
This can often be   difficult to read, especially from the command line. Being able to dump the
'cq4'  files as text , can also help you fix your production issues, as it gives you the
visibility of what has been   stored into the queue and in what order. 

The example below shows how to read a cq4 file from the command line:

You have to use the chronicle-queue.jar, from any version 4.5.3 or later, you have to use a set
of jars that work together, to find which jars work together please refer to the chronicle-bom,
which list sets of chroncile jars that work together.

You have to make sure that all the dependent jars ( to run chroncile queue ) are on you class path.
below is the set of jar that I am using along with the 'cq4' data file.
...
this will dump the 19700101-02.cq4 file out as text
