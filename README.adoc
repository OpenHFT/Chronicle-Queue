...
To write data to a chroncie-queue, you must first create an Appender
...
So, Chronicle-Queue uses and Appender to write to the queue and a Tailer to read from the queue. Unlike other java queues messages are not lost when they are read with tailer, This is covered in more detail in the second below on "Reading from a Queue"
