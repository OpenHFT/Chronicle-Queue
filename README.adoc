...
`Pretoucher` is a class designed to be called from a long-lived thread. The purpose of Pretoucher
...
release any other resources. Invocation of the `execute()` method after `shutdown()` has been called with
cause an `IllegalStateException` to be thrown.

The configuration parameters of the Pretoucher is as follows:

- `earlyAcquireNextCycle` (defaults to false): Causes the Pretoucher to create the next cycle file while the queue
...
- `pretoucherPrerollTimeMs` (defaults to 2,000 milliseconds) The pretoucher will create new cycle files
...
- `dontWrite` (defaults to false): Tells the Pretoucher to never create cycle files that do not already
