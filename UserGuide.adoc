...
- messages are grouped by *topics* A topic can contain any number of *sub-topics* which are logically stored together in that topic.
- an *appender* is the source of messages.
- a *tailer* is a receiver of messages.
- *Chronicle Queue* in broker-less by default. You can use *Chronicle Engine* to act as a broker for remote access.

NOTE: We deliberately avoid the term *consumer* as messages are not consumed/destroyed by reading.

At a high level, *appenders* append to a queue and, *tailers* keep reading from the next available message.
...
Within a topic you can have sub-topics which can be anything data type which can be serialized.
...
If you want to be able to upgrade your system you want to replay for the output.
...
Replication has three levels of gaurantees
...
In particular, a user can only take in som much data. For this reason it makes sense for the client consumer to tell the producer when and when not to give them more data.
It should determine how flow control works.
...
As such it is a power tool as a big buffer between your system and an upstream producer you have little or no control over.

=== Market Data

When you receive market data from a publisher you don't have the option to push back on the producer for long if at all. 
...
with very little buffering in the network adapter to save you time.
...
in addition, it also keeps a record of every message passed.

=== Compliance Systems

Complicated Systems are required by more and more systems these days.  Everyone has to have them but no one wants to be slowed down by them.
...
of complicated recording for your monitored systems.
...
Chronicle Queue supports low latency IPC between JVMs on the same machine ~ 1 micro-second, and between machines with a typical latency of 10 micro-seconds for
...
Streaming components are highly performant, deterministic and reproducable.
...
- without self describing messages, users had to create their own functionality for dumping messages and long term storage of data.  With Q4 you dont have to do this but is you wish you can still do so.
