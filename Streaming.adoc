...
List<News> newsList = Streams.of(
                    q.createTailer(),
                    builder(News.class).withMethod(Messages.class, Messages::news) <1>
            )
...
Note:: Some type parameters are not strictly needed but are there to aid Java's type inference.
...
It is possible to create an `ExcerprExtractor` that is reusing objects. Care must be taken if such an extractor is used so that, for example, reused objects are not exposed and/or not stored internally in the Streams pipeline. If in doubt, make a copy or extract an immutable value from the object at hand.

[source,java]
----
OptionalDouble max = Streams.of(queue.createTailer(),
               builder(MarketData.class)
                        .withReusing(new MarketData())   <1>
                        .build())
        .mapToDouble(MarketData::last)                   <2>
        .max();
----
<1> This object is reused when successively extracting a plurality of elements.
...
----
<1> This object is reused when successively extracting a plurality of elements.
<2> As objects are reused, the list will be populated with identical objects that will reflect the
    latest value of the reused object.
...
* Rename mapping() to groupingBy() or merging() ??