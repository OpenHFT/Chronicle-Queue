...
[#_queue_documents]
== Queue documents
A queue message comprise two parts, a 4-byte header followed by an link:../../getting-started/pages/glossary.adoc#e[excerpt]. The excerpt , also referred to as the message, contains the actual data, which could be of any type, including text, numbers, or serialised blobs. Regardless of the type, all information is stored as a series of bytes in the excerpt.
