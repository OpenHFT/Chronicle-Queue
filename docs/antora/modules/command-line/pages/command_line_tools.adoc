...
Where noted, each command line tool can be invoked via a shell script.
Command line tools can also be invoked using Maven, assuming a project POM file that includes the Chronicle Queue artifact.
For example:
...
The command-line utility is supported on Linux.

NOTE: `-dryrun` skips the final roll-file moves/deletions and QueueBackedMap snapshot write.
It still opens the source queue for writing during setup, so it is not a read-only inspection mode.
Use a disposable copy when investigating its actions.
The preview can continue after a non-removable file where an actual run would stop; it is not an exact prediction of a later run.
...
----
usage: ArchiveRollFilesMain -aa <archive-action> [-ad <archive-dir>] [-dryrun] [-h] [-hid <host-id>] [-kc <key-class>] [-m <min-archive-age>] [-qbme <qbm-entries>] [-qbmn
       <qbm-name>] [-qc <queue-config-file>] [-qd <queue-dir>] [-qn <queue-name>] [-r <roll-cycles-to-keep>] [-vc <value-class>] [-z <roll-timezone>]
 ...
 -hid <host-id>             Host ID in config file for current host. Required if -qc is specified.
 ...
 -qc <queue-config-file>    QueueReplicationConfig file containing queue configuration. Either -qc or -qd must be specified.
 -qd <queue-dir>            Directory containing queue files to archive. Either -qc or -qd must be specified.
 -qn <queue-name>           Name of queue as used in queue-config-file. Required if -qc is specified.
 ...
 -z <roll-timezone>         Time zone used for roll files. eg. 'Europe/Paris'. If not specified, time zone indicated specified in queue configuration is used, or defaults to UTC.
----
...
| -qc | Full or relative path to file to load configuration from for queue being archived. | If `-qd` is not specified
| -qn | Name of queue being archived, as it appears in file specified by -qc. eg. `'queue1'` | If `-qc` is specified
| -qd | Full or relative path to the directory containing the queue files being archived. Cannot be combined with `-qc`. | If `-qc` is not specified
| -hid | Host ID in the file specified by `-qc` identifying the current host, used to locate the queue directory. | If `-qc` is specified
...
| -dryrun | Log candidate actions without performing the final moves/deletions or map snapshot write; source-queue setup can still write. See the limitation above. | N
| -z | Time zone used for roll files, eg. `'Europe/Paris'`. If not specified, the time zone indicated in the queue configuration is used, or defaults to UTC. | N
|===
...
-Dexec.args="-qc config/queue-config.yaml -hid 1 -qn queue1 -m 2024-11-01T12:15:00 -aa DELETE"
----
...
----
$  mvn exec:java -Dexec.mainClass="software.chronicle.enterprise.queue.util.ArchiveRollFilesMain" \
-Dexec.args="-qc /tmp/qbm-replication.yaml -hid 1 -qn queue1 -m 2024-05-23T23:55:01 -aa ARCHIVE
...
----
$  mvn exec:java -Dexec.mainClass="software.chronicle.enterprise.queue.util.ArchiveRollFilesMain" \
-Dexec.args="-qc /tmp/qbm-replication.yaml -hid 1 -qn queue1 -r 3 -aa ARCHIVE -ad /mnt/share/queue-archive"
----
...
----
usage: ChronicleHistoryReaderMain -d <directory> [-h] [-i <ignore>] [-m] [-p] [-t <time unit>] [-u <histo offset>] [-w <window>]
...
`ChronicleReaderMain` can be used to log the contents of a chronicle queue, and is able to perform several operations beyond printing the file content to the console.
...
----
usage: ChronicleReaderMain [-a <binary-arg>] [-b <binary-search>] [-cbl <content-based-limiter>] [-cblArg <content-based-limiter-argument>] -d <directory> [-e <exclude-regex>] [-f]
       [-g] [-h] [-i <include-regex>] [-k] [-l] [-m <max-history>] [-n <from-index>] [-named <named>] [-r <as-method-reader>] [-s] [-w <wire-type>] [-x <max-results>] [-z]
...
NOTE: The generated help labels `-s` as "Display index", but the option actually suppresses the index prefix. Omit `-s` to display indexes.
...
This path can either represent a single .cq4 queue file, or a directory containing queue files.

NOTE: `DumpMain` is a tool intended for low-level debugging.
In most situations, <<_chroniclereadermain>> will provide more suited functionality for viewing the contents of queue files.
...
index2index: [
  # length: 4096, used: 1
  33008 # truncated trailing zeros
]
# position: 33008, header: -1
--- !!meta-data #binary
index: [
  # length: 4096, used:  # truncated trailing zeros
]
...
# 83820268 bytes remaining
----
...
`RefreshMain` can be used to refresh the directory listing for a queue, contained within the tablestore file for the queue.
RefreshMain takes a single parameter indicating the directory containing queue files.
...
`UnlockMain` can be used to forcibly unlock the a queue.
UnlockMain takes a single parameter indicating the directory containing queue to unlock.
