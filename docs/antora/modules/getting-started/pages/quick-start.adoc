...
[source]
----
env JAVA_HOME=/usr/lib/jvm/java-1.8.0-openjdk-amd64 PATH=$JAVA_HOME/bin:$PATH mvn clean install
----
...
The example below creates an `IndexedChronicle` which creates two `RandomAccessFiles`; one for indexes, and one for data having names relatively:
