...
This section helps you get started with Chronicle Queue in a few minutes.
...
The provided guide does not intend to be an exhaustive explanation of these steps but rather provide enough information to get you up and running quickly.
Hence, most of the topics are explained in greater depth throughout the Chronicle Queue documentation.
...
The installation is performed by notifying your build tool that the Chronicle Queue dependency is required to compile your source code.
Below are instructions for installation using Maven or Gradle.
...
A new Chronicle Queue is obtained from a builder by passing the field `singleBuilder` the path where the queue is to be stored.
The example below creates a `SingleChronicleQueue`, which stores data and index together in one memory-mapped `.cq4` file per roll cycle, alongside a `metadata.cq4t` file:
...
The queue files will reside in the directory `getting-started`.
With the default `FAST_DAILY` roll cycle, files are named with the current date followed by the cycle-type letter:

[source,shell script]
----
getting-started/{today}F.cq4
getting-started/metadata.cq4t
----

=== Writing to a Queue [[writing_to_a_queue]]

Messages are written, or appended, to a queue using an appender.
The appender writes messages to the end of the queue only, there is no way to insert messages at a specific location.

Here is how you can append a simple text message containing the text "Hello World!":

[source,java]
----
ExcerptAppender appender = queue.createAppender(); #<1>
...
Reading a from a queue is easiest done using a tailer, namely `ExcerptTailer`.
The term _tailing_ stems from reading from the queue's tail, i.e. the end of the queue.
However, you are not limited to only read from the end of the queue using the tailer.
...
IMPORTANT: No data will be lost when closing the queue.
This procedure only cleans up resources that were used at runtime.
