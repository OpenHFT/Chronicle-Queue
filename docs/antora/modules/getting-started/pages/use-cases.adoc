...
IMPORTANT: If using Chronicle Queue on NFS, there are a number of considerations to take into account.
See https://github.com/OpenHFT/Chronicle-Queue/blob/ea/docs/FAQ.adoc#creating-chronicle-queue-on-network-file-system-nfs-mount[this Chronicle Queue documentation] for more information.
If any networking is needed (e.g. to make the data accessible to multiple hosts), the recommended solution is to use Chronicle Queue Replication (Enterprise feature).

== Market Data

Market data publishers don't give you the option to push back on the producer for long; if at all.
A few of our users consume data from CME OPRA.
This produces peaks of 10 million events per minute, sent as UDP packets without any retry.
