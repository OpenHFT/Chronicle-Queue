...
=== Refreshing The Directory Listing For A Queue Using `RefreshMain`

`RefreshMain` can be used to refresh the directory listing for a queue, contained within the tablestore file for the queue. Assuming a project pom file that includes the Chronicle-Queue artifact, you can refresh the directory listing for a queue with the following command:
...
Where _myqueue_ is the directory containing your .cq4 files.
