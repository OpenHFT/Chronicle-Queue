...
If a process stops after removing EOF, back-fill is restarted and the entry in
flight is retried. When the restarted back-fill is complete, `normaliseEOFs()`
includes the historical cycle on which its appender is parked and reseals it.
This applies whether the retry writes the missing entry or finds that the entry
was already committed. An existing index is authoritative: Queue does not
overwrite it or compare the duplicate payload supplied by another sink.
