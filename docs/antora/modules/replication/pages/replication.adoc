...
writes_, not permanently immutable. Queue does not publish a durable
...
CQE owns that protocol for the supported replication use case: it retries a
failed exact-index back-fill after restart, calls `normaliseEOFs()` before
publishing back-fill completion, and keeps ordinary source writes excluded
until then. Another internal caller of exact-index `writeBytes` assumes those
same responsibilities. Queue construction and later ordinary appends can
repair some older EOFs, but are a fallback only; they are not a replacement for
caller-owned retry, normalisation and maintenance exclusion.
...
when they match, and warns with the two lengths and deterministic hashes when
they differ. The payload bytes are not included in the warning.
...
The duplicate check also permits a replication implementation to replay a
small overlap of recent entries to verify that source and sink agree before
transferring missing data. Current callers need not use that optional check.
...
lock. It does not depend on a new Wire EOF-recovery operation and works with the
currently released Wire API. Wire changes which clear transient
`insideHeader` state before an ordinary EOF exception are useful state hygiene,
but are not a prerequisite for this recovery path.
