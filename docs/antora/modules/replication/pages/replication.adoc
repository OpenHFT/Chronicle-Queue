...
unlocked for normal use. Other callers must provide equivalent coordination,
and external archival or cleanup tools must not treat an end-of-data marker as
proof that a roll can never be reopened.
...
constructs the Wire, before recovery can use it; the recovery method has no
unpadded mode. Padding therefore remains an invariant throughout recovery and
keeps every header, including the end-of-data word, four-byte aligned for atomic
replacement.
...
Reaching a roll's sparse-index capacity does not prevent exact-index recovery.
Queue warns when the first entry at the capacity boundary is written, leaves
that entry and later entries unindexed, and locates them by scanning forward
from the last addressable sparse-index entry.
