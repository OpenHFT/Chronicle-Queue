...
multi-entry back-fill emits at most the one EOF-reopen warning. An existing
published index remains authoritative: Queue compares a replayed payload,
returns silently when it matches, and warns without overwriting when it differs.
The warning identifies the queue, cycle, index and lengths but does not log
payload content. A ready first-writer record left beyond write-position
publication is adopted without overwriting its payload. Otherwise a new index
must be exactly next; a gap is rejected.
