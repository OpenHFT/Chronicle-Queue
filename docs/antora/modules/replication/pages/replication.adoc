...
published index remains authoritative and is not accepted by this recovery
path. A ready first-writer record left beyond write-position publication is
adopted without comparing or overwriting its payload. Otherwise the requested
index must be exactly next; an existing index or a gap is rejected.
