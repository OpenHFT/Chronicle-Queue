...
Exact-index recovery is supported only for padded stores. Queue scans complete
data, user metadata and sparse-index metadata to locate EOF, replaces that word
with one compare-and-swap, commits the data and required index entry, and then
restores EOF. An existing index is authoritative: Queue does not overwrite it
or compare the duplicate payload supplied by another sink.
