...
It is possible to cap the total number of index arrays per queue file, hence also determine the size of each index array.
This is done by configuring the parameter `indexCount(int indexCount)` when building the queue.
...
The sparse index can address `indexCount`^2^ × `indexSpacing` entry sequences
in a roll. Reaching that boundary produces a warning but does not prevent later
entries from being written. Those entries remain unindexed and random access
locates them by scanning forward from the final addressable sparse-index entry.
