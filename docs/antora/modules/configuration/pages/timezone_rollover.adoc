...
In the open-source version only `UTC` (or `Z`) is supported; any other `zoneId` logs a warning and causes `build()` to throw `IllegalStateException`, due to configurable time zone rollover being an Enterprise feature.
...
The following code extract uses `America/New_York` as the time zone and rolls over queues at 17:00 local time.
