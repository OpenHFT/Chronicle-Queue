...
In the open-source version only `UTC` (or `Z`) is support; any other `zoneId` logs a warning and causes `build()` to throw `IllegalStateException`, due to configurable time zone rollover being an Enterprise feature.
