...
=== Runnable Demos
Chronicle has gathered all runnable code demonstrations on GitHub for easy access. The demos are great examples of how the learnings from the user guide can be put into practise. You can request access to the demos link:https://chronicle.software/demo/?product=queue[here]. 
...
In case you are not yet interested in integrating Queue in one of your projects, you may want to start out by running one of our demos. You find our demonstrations link:https://chronicle.software/demo/?product=queue[here].

== Chronicle Queue Enterprise
Chronicle Queue Enterprise is a commercially supported version of Chronicle Queue OSS. This documentation covers the open source functionality but is also extended with a few chapters describing enterprise specific features. These sections are clearly marked with a star-symbol (★) and include:
