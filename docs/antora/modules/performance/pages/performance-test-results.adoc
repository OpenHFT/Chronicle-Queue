...
Note, the C{pp} plot uses a linear scale for the latency axis; the Java plot uses a logarithmic scale.
...
Further details of Queue Zero are available by contacting mailto:sales@chronicle.software[sales@chronicle.software]
...
To make the comparison between Chronicle Queue ( abbreviated to Queue ) and Chronicle Queue Zero ( abbreviated to Queue Zero ) performance even clearer, the below plots show the end-to-end latencies for sending a 1024-byte message using Queue and Queue Zero, for C++ and Java respectively.
These plots again show both the much reduced latency using Queue Zero, as well as the reduction of outliers at higher percentiles.
...
* These results where generated using `software.chronicle.enterprise.queue.stress.QueueMultiThreadedJLBHBenchmark` (Chronicle Queue Enterprise) for Java, and `C{pp}/test/queue/bench/QueueMultiThreadedJLBHBenchmark.cpp` for C{pp}.
