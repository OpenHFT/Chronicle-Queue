...
To make the comparison between Chronicle Queue ( abbreviated to Queue ) and Chronicle Queue Zero ( abbreviated to Queue Zero ) performance even clearer, the below plots show the end-to-end latencies for sending a 1024-byte message using Queue and Queue Zero, for C{pp} and Java respectively.
...
* These results where generated using `net.openhft.chronicle.queue.bench.QueueMultiThreadedJLBHBenchmark` for Java, and `C{pp}/test/queue/bench/QueueMultiThreadedJLBHBenchmark.cpp` for C{pp}.
