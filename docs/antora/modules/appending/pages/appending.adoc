...
Chronicle refers to the act of appending (writing) data to a Chronicle Queue, as storing an excerpt.
This data could contain any data types, including text, numbers, or serialised blobs.
Ultimately, regardless of what it is, all data is stored as a series of bytes.

Prior to storing your excerpt, a 4-byte header is reserved by the queue.
This header contains, among other things, the length of the data in order for readers to know the size of the blob.
Onwards, we refer to this 4-byte header, along with your excerpt, as a link:../../getting-started/pages/glossary.adoc#d[document].
Strictly speaking, Chronicle Queue is used to read and write such documents.

NOTE: A few bits in the header are reserved for a number of internal operations, such as locking, to ensure thread-safety across both processors and threads.
For this reason, converting the header to an integer does *not* yield the length of the data blob.
...
Multiple YAML files can be passed in one invocation of `ChronicleWriterMain`; they are written sequentially so you can batch related method calls without spawning a new JVM for each payload.

TIP: Refer to xref:command-line:command_line_tools.adoc#_chroniclewritermain[] for the full CLI reference, including the Maven and shell-script entry points.
