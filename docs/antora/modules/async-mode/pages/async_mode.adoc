...
mitigate pauses which the OS can introduce when using `ChronicleQueue` under load.
Using Enterprise Chronicle
...
As the Chronicle stack is built on `Bytes`, you can use the buffer for byte arrays, packed objects, or you can serialise
...
The `BytesStore` must be an
implementation of `NativeBytesStore`, and must be sized appropriately by using the `sizeFor` method.
...
This use-case is designed to help with very "bursty" writes,
which cannot afford to be blocked by a slow underlying disk sub-system.
The asynchronous buffer size must be carefully
...
be given to the buffer and reads will read from the buffer.
This gives the lowest possible latency as the
disk subsystem is not involved at all.
As with asynchronous writes, a background task is created in the builder's event
...
If you do not set this, and try and access
...
from some methods.
It does not contain the full functionality of EnterpriseSingleChronicleQueue - it does not
drain to an underlying queue, for example.
...
The exception may be thrown at either read or write time depending on how
