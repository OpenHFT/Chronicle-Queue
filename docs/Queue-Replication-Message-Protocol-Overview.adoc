...
== Overview
This document covers the high level protocol and handshaking that is used by chronicle queue enterprise message data via tcp/ip to replication queues between different machines.

== Audience

Anyone who wants to get a bit more inside into the data transefered between the remotes host for chronicle queue enterprise tcp/ip replication. This document is not a queue functionally overview.
