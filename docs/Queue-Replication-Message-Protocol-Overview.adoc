...
== Overview
This document covers the high level protocol and handshaking that is used by chronicle queue enterpise to do tcp/ip data replication between queue running on differnt machine.

== Audience

Anyone who wants to get a bit more inside into the data transefer between the remotes host of queue repliaction. This document is not a user functionally guide.
...
--- !!meta-data #binary
cid: 292323922173§575
--- !!data #binary
# position: 17, header: 0
--- !!data #binary
idx: 0x451600000000
ns: 10849029994071
