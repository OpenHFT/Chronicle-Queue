...
The main idea is to manage your memory manually so it does not suffer from GC. Chronicle behaves like a management interface over off-heap memory, so that you can build your own solutions over it.
...
data belonging to a single _cycle_.
The length of the cycle is determined by
...
described above (since the structure needs to be written to).
In this case,
...
old files.
An automated process needs to ensure that there are not active
...
persisted roll-cycle.
In this case, a warning log message will be printed in order to notify the library user
of the situation:
