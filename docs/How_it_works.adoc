...
try (final ChronicleQueue queue = SingleChronicleQueueBuilder.binary(queueDir).rollCycle(SECONDLY).build()) {
...
// this call will fail since we are trying to create a new queue, at the same location, with a different roll-cycle
try (final ChronicleQueue recreated = SingleChronicleQueueBuilder.binary(queueDir).rollCycle(HOURLY).build()) {
...
try (final ChronicleQueue minuteRollCycleQueue = builder(queuePath, wireType).rollCycle(MINUTELY).build()) {
    // creates a queue with roll-cycle HOURLY - valid since no appends have yet been made
    try (final ChronicleQueue hourlyRollCycleQueue = builder(queuePath, wireType).rollCycle(HOURLY).build()) {
