...
It's possible to use Chronicle Queue to invoke a method on the other JVM and wait for the return value. Imagine a simple scenario with two processes: `C` (client) and `S` (server). Create two `IndexedChronicles`:
