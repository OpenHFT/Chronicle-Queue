...
ExcerptAppender appender = queue.createAppender(); // sequential writes.
...
[source]
----
env JAVA_HOME=/usr/lib/jvm/java-1.8.0-openjdk-amd64 PATH=/usr/lib/jvm/java-1.8.0-openjdk-amd64/bin:$PATH mvn clean install -l build-java8.log
env JAVA_HOME=/usr/lib/jvm/java-1.21.0-openjdk-amd64 PATH=/usr/lib/jvm/java-1.21.0-openjdk-amd64/bin:$PATH mvn clean install -l build-java21.log
----
