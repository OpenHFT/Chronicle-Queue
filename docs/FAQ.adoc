...
The target round-trip time for persisting the request, processing, and persisting the response, was `1` micro-second.
...
If you use Wire, the typical latencies tend to be around one micro-second.
...
Chronicle Queue is designed to persist messages, and replay them in micro-second time.
Simple messages take as low as `0.4` micro-seconds.
...
== Diagnosing method-writer compilation fallbacks in tests

When Wire records `Failed to compile generated method writer - falling back to proxy method writer`,
tests extending `QueueTestCommon` print additional context to standard error when teardown starts.
The diagnostic includes the test class and method, Java version, `wire.generator.v2` and
`disableProxyCodegen` properties, and the original throwable with its cause and stack trace.
An unset property is shown as `null`; this does not describe its effective default.
Extra output is limited to the first distinct matching warning and its recorded occurrence count.
The original warnings remain subject to the existing exception checks, and diagnostics also run
for failed tests and tests which set `finishedNormally` to false.

Rerun the affected test on the same JDK and retain its log and Surefire reports. For example,
if `MessageHistoryTest` was affected, run from the repository root:

[source,shell]
----
mvn -Dtest=MessageHistoryTest -DfailIfNoTests=true -Dsurefire.failIfNoSpecifiedTests=true test -l method-writer-fallback.log
----

Confirm that `target/surefire-reports` records the intended test execution. A successful rerun
without a fallback warning does not reproduce the original compilation failure.
