...
## Prototype result
A non-blocking `WatchService` wrapper proved that local `.cq4` create/delete events can be detected,
but no production consumer or latency requirement was identified. The prototype is therefore not
retained as public production code. The watcher and its integration must land together if the
consumer gate below is satisfied.
...
Provisional lean (pending the maintainer's consumer answer): **do not add an unwired component.** If a
real consumer with a sub-60s latency requirement is named, implement the watcher and its production
integration together per the design below; otherwise close #924 as not planned.
...
- If watch-service creation succeeds but directory registration fails, close the service before
  propagating the registration error.
- Drain or classify events generated by this Queue instance without hiding concurrent external
  changes; an ordinary local roll must not trigger a redundant full scan.
...
changed `.cq4` files, and what is their maximum acceptable latency?** Yes: build the watcher and
consumer together per the design above. No: close #924; the current mechanisms already suffice.
