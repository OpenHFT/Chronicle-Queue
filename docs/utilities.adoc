...
You can specify arguments that tailor the way that the Chronicle Reader utility behaves.
For example:
...
Following a queue ensures that the latest messages are written out to `Standard-Out`.
It will wait, and append new messages as they arrive on the queue.
...
== Roll File Cleanup utility

For named-tailer-position-based roll-file retention and operational clean-up, see link:roll-file-cleanup.adoc[Roll File Cleanup].
