...
Using `-r` will cause the Reader to treat each event within a Document Excerpt as a single message
for the purposes of applying regular expressions.
...
For convenience, this tool back be packaged for deployment to a server environment.

To create an archive for deployment, run the following from the project root:
...
The resulting tar file can be copied to another location & expanded.
