...
(as indicated by `masterId`) to all other hosts defined for the cluster, without acknowledgemnt.
...
This can be acheived after committing a write (e.g. closing `DocumentContext`) by this loop
```
