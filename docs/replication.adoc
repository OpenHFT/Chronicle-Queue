...
Chronicle Queue automatically performs replication if it finds this replication artifact in the class path.
...
<3> specifies the point in the queue where the next level of indexing begins.
...
<2> `data` defines the location as the start of the data.
