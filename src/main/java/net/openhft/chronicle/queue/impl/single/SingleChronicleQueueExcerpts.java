...
public class SingleChronicleQueueExcerpts {
	...
	static class StoreAppender implements ExcerptAppender, ExcerptContext, InternalAppender {
		...
		/**
		 * @param padToCacheLines the default for chronicle queue is Padding.SMART, which automatically pads all method calls other than
		 *            {@link StoreAppender#writeBytes(net.openhft.chronicle.bytes.WriteBytesMarshallable)} and
		 *            {@link StoreAppender#writeText(java.lang.CharSequence)}. Which can not be padded with out changing the message format, The
		...
		boolean checkWritePositionHeaderNumber() {
			if (wire == null || wire.headerNumber() == Long.MIN_VALUE)
				return true;
			try {
				long pos1 = position;
				/*
		...
		private <T> void append(int length, @NotNull WireWriter<T> wireWriter, T writer) throws UnrecoverableTimeoutException {
		...
		/**
		 * Write an EOF marker on the current cycle if it is about to roll. It would do this any way if a new message was written, but this
		 * doesn't create a new cycle or add a message.
		 */
		...
		<T> void append2(int length, @NotNull WireWriter<T> wireWriter, T writer)
				throws UnrecoverableTimeoutException, EOFException, StreamCorruptedException {
	...
	public static class StoreTailer implements ExcerptTailer, SourceContext, ExcerptContext {
		...
		public boolean readDocument(@NotNull ReadMarshallable reader) {
			try (@NotNull
			DocumentContext dc = readingDocument(false)) {
				if (!dc.isPresent())
					return false;
		...
		private boolean inACycle(boolean includeMetaData, boolean first)
			...
			if (readAfterReplicaAcknowledged && inACycleCheckRep())
				return false;

			if (direction != TailerDirection.FORWARD && inACycleNotForward())
				return false;
		...
		private long nextIndexWithNextAvailableCycle(int cycle) {
			...
			if (nextIndex != Long.MIN_VALUE && queue.rollCycle().toCycle(nextIndex) - 1 != cycle) {
				...
				/**
				 * lets say that you were using a roll cycle of TEST_SECONDLY and you wrote a message to the queue, if you created a tailer and
				 * read the first message, then waited around 22 seconds before writing the next message, when the tailer came to read the next
				 * message, there would be a gap of 22 cycle files that did not exist, that is what this is reporting. If you are using daily
				 * rolling, and writing every day, you should not see this message.
				 */
		...
		ScanResult moveToIndexResult(long index) {
			...
			System.out.println("Special case here that will happend when the tests fails");
			bytes.readLimit(bytes.readPosition());
			return scanResult;
		}
		...
		/**
		 * gives approximately the last index, can not be relied on as the last index may have changed just after this was called. For this
		 * reason, this code is not in queue as it should only be an internal method
		...
		public void lastAcknowledgedIndexReplicated(long acknowledgeIndex) {
			...
			Jvm.debug().on(getClass(), "received lastAcknowledgedIndexReplicated=" + Long.toHexString(acknowledgeIndex) + " ,file="
			...
			try {
				...
				if (!temp.cycle(cycle0, false)) {
					Jvm.warn().on(getClass(),
							"Got an acknowledge index " + Long.toHexString(acknowledgeIndex) + " for a cycle which could not found");
					return;
				}
		...
		public void setCycle(int cycle) {
			...
			timeForNextCycle = cycle == Integer.MIN_VALUE ? Long.MAX_VALUE : (long) (cycle + 1) * queue.rollCycle().length() + queue.epoch();
		...
		private static final class MoveToState {
			...
			private long calculateKnownIndex(final long readPosition) {
				return readPositionAtLastMove != readPosition ? lastMovedToIndex + 1 : lastMovedToIndex;
			}
